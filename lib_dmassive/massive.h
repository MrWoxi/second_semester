...
TDMassive<T>& TDMassive<T>::insert(const T* arr, size_t n, size_t pos) {
    if (_size < pos) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& insert(const T* arr, size_t n, size_t pos)\": wrong position value.");
    }
...
void TDMassive<T>::pop_front() {
    if (_size <= 0) {
        throw std::logic_error("Error in function \
\"void TDMassive<T>::pop_front()\": archive clear");
    }
...
void TDMassive<T>::pop_back() {
    if (_size <= 0) {
        throw std::logic_error("Error in function \
\"void TDMassive<T>::pop_back()\": archive clear");
    }
...
TDMassive<T>& TDMassive<T> ::remove_by_index(size_t pos) {
    if (_size <= pos || pos < 0) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& TDMassive<T> ::remove_by_index(size_t pos)\": wrong position value.");
    }
...
TDMassive<T>& TDMassive<T>::erase(size_t pos, size_t n) {
    if (_size < pos) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& TDMassive<T>::erase(size_t pos, size_t n)\": wrong position value.");
    }
...
TDMassive<T>& TDMassive<T>::remove_all(T value) {
    if (_size <= 0) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& TDMassive<T>::remove_all(T value)\": archive clear");
    }
    for (size_t i = 0; i < _size; i++) {
        if (_data[i] == value) {
            _states[i] = State::deleted;
...
TDMassive<T>& TDMassive<T>::remove_first(T value) {
    if (_size <= 0) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& TDMassive<T>::remove_first(T value)\": archive clear");
    }
    for (size_t i = 0; i < _size; i++) {
        if (_data[i] == value) {
            _states[i] = State::deleted;
...
TDMassive<T>& TDMassive<T>::remove_last(T value) {
    if (_size <= 0) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& TDMassive<T>::remove_last(T value)\": archive clear");
    }
    for (size_t i = _size; i > 0; i--) {
        if (_data[i] == value) {
            _states[i] = State::deleted;
...
size_t TDMassive<T>::find_first(T value) const{
    ...
    throw std::logic_error("Error in function \
\"size_t TDMassive<T>::find_first(T value)\":No mathes");
}
...
TDMassive<T>& TDMassive<T>::replace(size_t pos, T new_value) {
    if (_data[pos] != State::busy) {
        throw std::logic_error("Error in function \
\"TDMassive<T>& replace(size_t pos, T new_value)\":No mathes");
    }
    if(_data[pos] )
    _data[pos] = new_value;
    _states[pos] = State::busy;
